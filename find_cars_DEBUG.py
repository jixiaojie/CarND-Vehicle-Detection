...
def find_cars(image, skip = 8):
    ...
    if (labels is None and loop_num == 1) or (loop_num % skip == skip - 1):
        ...
        mpimg.imsave('output_images/test_heatmap' + str(loop_num) +'.jpg', heatmap, cmap ='gray')        

        # Find final boxes from heatmap using label function
        labels = label(heatmap)
        mpimg.imsave('output_images/test_labels' + str(loop_num) +'.jpg', draw_labeled_bboxes(np.copy(heatmap), labels), cmap ='gray')
        #draw_boxes(img, bboxes, color=(0, 0, 255), thick=6)
        mpimg.imsave('output_images/test_hotwindows' + str(loop_num) +'.jpg', draw_boxes(np.copy(image), hot_windows))
        mpimg.imsave('output_images/test_slidewindows' + str(loop_num) +'.jpg', draw_boxes(np.copy(image),window_slid ))
...
if i == 5:
    #image = mpimg.imread('output_images/car.png')
    image = cv2.cvtColor(cv2.imread('output_images/car.png'),cv2.COLOR_BGR2RGB)
    mpimg.imsave('output_images/car-R.jpg', image[:,:,0], cmap ='gray')
    mpimg.imsave('output_images/car-G.jpg', image[:,:,1], cmap ='gray')
    mpimg.imsave('output_images/car-B.jpg', image[:,:,2], cmap ='gray')

    _, img = get_hog_features(image, orient,
                        pix_per_cell, cell_per_block, vis=True, feature_vec=True)
    mpimg.imsave('output_images/car-hot.jpg', img, cmap = 'gray')

    _, img = get_hog_features(image[:,:,0], orient,
                        pix_per_cell, cell_per_block, vis=True, feature_vec=True)
    mpimg.imsave('output_images/car-R-hot.jpg', img, cmap ='gray')

    _, img = get_hog_features(image[:,:,1], orient,
                        pix_per_cell, cell_per_block, vis=True, feature_vec=True)
    mpimg.imsave('output_images/car-G-hot.jpg', img, cmap ='gray')

    _, img = get_hog_features(image[:,:,2], orient,
                        pix_per_cell, cell_per_block, vis=True, feature_vec=True)
    mpimg.imsave('output_images/car-B-hot.jpg', img, cmap ='gray')

   
    image = cv2.cvtColor(cv2.imread('output_images/notcar.png'),cv2.COLOR_BGR2RGB)
    mpimg.imsave('output_images/notcar-R.jpg', image[:,:,0], cmap ='gray')
    mpimg.imsave('output_images/notcar-G.jpg', image[:,:,1], cmap ='gray')
    mpimg.imsave('output_images/notcar-B.jpg', image[:,:,2], cmap ='gray')

    _, img = get_hog_features(image, orient,
                        pix_per_cell, cell_per_block, vis=True, feature_vec=True)
    mpimg.imsave('output_images/notcar-hot.jpg', img, cmap ='gray')

    _, img = get_hog_features(image[:,:,0], orient,
                        pix_per_cell, cell_per_block, vis=True, feature_vec=True)
    mpimg.imsave('output_images/notcar-R-hot.jpg', img, cmap ='gray')
    
    _, img = get_hog_features(image[:,:,1], orient,
                        pix_per_cell, cell_per_block, vis=True, feature_vec=True)
    mpimg.imsave('output_images/notcar-G-hot.jpg', img, cmap ='gray')

    _, img = get_hog_features(image[:,:,2], orient,
                        pix_per_cell, cell_per_block, vis=True, feature_vec=True)
    mpimg.imsave('output_images/notcar-B-hot.jpg', img, cmap ='gray')
